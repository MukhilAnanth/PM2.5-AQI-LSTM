...
while current_date <= end_date:
    ...
    AQI = pd.to_numeric(history.iloc[:, 3], errors="coerce").dropna().values

    if len(AQI) < TIME_STEPS:
        current_date += timedelta(days=1)
        continue

    AQI_scaled = scaler.transform(AQI.reshape(-1, 1))

    last_sequence = AQI_scaled[-TIME_STEPS:].reshape(1, TIME_STEPS, 1)

    pred_scaled = model.predict(last_sequence, verbose=0)
    pred_aqi = scaler.inverse_transform(pred_scaled)[0][0]
    actual_row = data[data.iloc[:, 0] == current_date]
    if not actual_row.empty:
        actual_aqi = pd.to_numeric(
            actual_row.iloc[0, 3], errors="coerce"
        )

        if not np.isnan(actual_aqi):
