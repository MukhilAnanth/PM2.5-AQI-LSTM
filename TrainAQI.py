...
print(data.columns)
AQI = pd.to_numeric(data.iloc[1:2197, 3], errors='coerce').dropna().values
print("Number of valid samples:", len(AQI))



scaler = MinMaxScaler()
AQI_scaled = scaler.fit_transform(AQI.reshape(-1, 1))
...
X, y = create_sequences(AQI_scaled, TIME_STEPS)
